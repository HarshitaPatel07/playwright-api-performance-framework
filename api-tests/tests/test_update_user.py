...
class TestUpdateUserPositive:
    ...
    def test_patch_does_not_affect_other_fields(self, user_to_update):
        ...
        data = response.json()
        assert data["name"] == UPDATE_USER_NAME["name"]
        assert data["email"] == user3["email"]
        assert data["gender"] == user3["gender"]
        assert data["status"] == user3["status"]
